...
class bool_def:public arx::referrable
{
    public:
    bool_def(std::shared_ptr<arx::armodel> model, const std::string& shortname, std::unique_ptr<std::string> uuid, rapidxml::xml_node<>* node);
    
    multiplicity_t lower_multiplicity() { return _lower_multiplicity; }
    multiplicity_t upper_multiplicity() { return _upper_multiplicity; }
    bool default_value() { return _def_val; }
